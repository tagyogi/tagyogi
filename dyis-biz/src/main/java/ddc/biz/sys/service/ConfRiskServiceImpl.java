...
import ddc.biz.sys.repository.ConfRiskMapper;
 ...
 *  </pre>
 */
@Service("confRiskService")
@Transactional(propagation = Propagation.REQUIRED, rollbackFor = { Exception.class })
public class ConfRiskServiceImpl implements ConfRiskService {

	private static final Logger LOGGER = LoggerFactory.getLogger(ConfRiskServiceImpl.class);
	
	@Autowired(required = false)
	private ConfRiskMapper confRiskMapper;

	//리스크및환경 현황 조회
	@Transactional(readOnly = true)
	public List<Map<String, Object>> selectConfRiskList(HashMap paramMap) {
		return confRiskMapper.selectConfRiskList(paramMap);
	}

	//리스크및환경 상세 조회(최종)
	@Transactional(readOnly = true)
	public HashMap selectConfRiskMaxAppDt(HashMap paramMap) {
		return confRiskMapper.selectConfRiskMaxAppDt(paramMap);
	}
