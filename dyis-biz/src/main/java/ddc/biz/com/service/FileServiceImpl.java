...
public class FileServiceImpl implements FileService {
	...
	public void fileUpload(HashMap paramMap, List<MultipartFile> fileList) {
		...
		if(atthNo != null) {
			...
			for (int i = 0; i < getfileList.size(); i++) {
				HashMap<String, Object> getMap = (HashMap)getfileList.get(i);
	...
	public void deleteFileMaster (HashMap paramMap) {
		...
		if(!"null".equals(atthNo)) {
			...
			List fileList = fileMapper.selectFileList(paramMap);
			...
			for (int i = 0; i < fileList.size(); i++) {
				...
				//파일삭제
				HashMap fileMap = fileMapper.selectFile(paramMap); //파일정보조회
