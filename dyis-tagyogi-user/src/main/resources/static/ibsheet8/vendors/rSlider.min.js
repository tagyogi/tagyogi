(function () {
	...
	RS.prototype.createScale = function (resize) {
		...
		for (var i = 0, iLen = this.conf.values.length; i < iLen; i++) {
			...
			if (!this.conf.labels) {
				if (i === 0 || i === iLen - 1) ins.innerHTML = this.conf.labels[i]
			}
			else ins.innerHTML = this.conf.labels[i];
	...
	RS.prototype.updateScale = function (resize) {
		this.step = this.sliderWidth / (this.conf.values.length - 1);

		var pieces = this.slider.querySelectorAll('span');

		for (var i = 0, iLen = pieces.length; i < iLen; i++) {
			if(i !== (iLen - 1)) pieces[i].style.width = this.step + 'px';
    }

		return this.setValues(null, null, resize);
	};
	...
	RS.prototype.setValues = function (start, end, resize) {
	...
	RS.prototype.onResize = function () {
		/*
			resize 인자를 true 로 줘서 resize 시에는 onChange 이벤트가 발생하지 않도록 합니다.
			현재 열린 창의 크기에 따라 Slider를 감추거나 숨깁니다.
		*/
		var winWidth = $(window).width();
		var winHeight = $(window).height();

		if(winWidth < 850 || winHeight < 650) {
