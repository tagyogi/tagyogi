...
function f_calendar(ctl, ctl2, format, fromDate, pos) {
    ...
    if(bPageLoaded) {
        ...
        ctop = document.body.clientTop + GetObjectTop(ctl) - document.body.scrollTop+40;
        cleft = document.body.clientLeft + GetObjectLeft(ctl) -  document.body.scrollLeft- 205;

        //달력이 하단에 있으면 잘림 방지
        if(ctop > com.fnToNum(document.body.clientHeight + document.documentElement.scrollTop / 2)) ctop = com.fnToNum(ctop - 235);
