...
import ddc.util.FileUploadUtil;
...
public class DevDbController {
	...
	public ModelAndView devDbInit(HttpServletRequest request, @RequestBody(required=false) HashMap paramMap) throws Exception {
		...
		try{
			...
			HashMap getTbl = devDbService.selectTblInfo(paramMap);
	...
	public ModelAndView devDbListPage(HttpServletRequest request, @RequestBody(required=false) HashMap paramMap) throws Exception {
	...
	public ModelAndView selectDevDbMasList(HttpServletRequest request, @RequestBody(required=false) HashMap paramMap) throws Exception {
		ModelAndView josnView = new ModelAndView("jsonView");
	...
	public ModelAndView selectDevDbDtlList(HttpServletRequest request, @RequestBody(required=false) HashMap paramMap) throws Exception {
		ModelAndView josnView = new ModelAndView("jsonView");
	...
	public ModelAndView saveDevDbMasProc(@RequestBody(required=false) HashMap paramMap) throws Exception {
		ModelAndView josnView = new ModelAndView("jsonView");
	...
	public ModelAndView saveDevDbDtlProc(@RequestBody(required=false) HashMap paramMap) throws Exception {
		ModelAndView josnView = new ModelAndView("jsonView");
	...
	public ModelAndView deleteDevDbProc(@RequestBody(required=false) HashMap paramMap) throws Exception {
		ModelAndView josnView = new ModelAndView("jsonView");
	...
	public ModelAndView insertColsInitProc(@RequestBody(required=false) HashMap paramMap) throws Exception {
		ModelAndView josnView = new ModelAndView("jsonView");
	...
	public ModelAndView createTblProc(@RequestBody(required=false) HashMap paramMap) throws Exception {
		ModelAndView josnView = new ModelAndView("jsonView");
		...
		try{
			...
			//테이블 정보 조회
	    	HashMap masMap = devDbService.selectDevDbMasInfo(paramMap);
	    	
	    	//테이블 필드 정보 조회
	    	List dtlList = devDbService.selectDevDbDtlList(paramMap);
	    	
	    	//테이블 이전 필드 정보 조회
	    	List befDtlList = devDbService.selectBefTblColList(paramMap);
	    	
	    	//테이블 생성 쿼리문 추출
	    	HashMap getSql = createSql(DB_TYPE, masMap, dtlList, befDtlList);
    ...
    // 테이블 생성 명령어 구현
    public HashMap createSql(String DEV_DB_TYPE, HashMap masMap, List dtlList, List befDtlList) throws Exception {
    	...
    	if("oracle".equals(DEV_DB_TYPE)) {
    		...
    		for(int i = 0; i < dtlList.size(); i++) {
    			HashMap setDtlMap = (HashMap) dtlList.get(i);
    		...
    		for(int i = 0; i < befDtlList.size(); i++) {
    			HashMap setBefMap = (HashMap) befDtlList.get(i);
	...
	public void exportTblExcel(HttpServletResponse response, HttpServletRequest request, @RequestBody(required=false) HashMap paramMap) throws Exception {
		...
		try{
			...
			String[] sHeads = null;
			//헤더 생성
			List colsList = devDbService.selectBefTblColList(paramMap);
			objRow = (SXSSFRow) objSheet.createRow(rowNum++);
    		
			for (int x = 0; x < colsList.size(); x++) {
				HashMap getMap = (HashMap)colsList.get(x);
			...
			//데이터
			List<HashMap> excelList = devDbService.selectTblList(paramMap);
	...
	public ModelAndView importTblExcel(HttpServletRequest request) throws Exception {
		JsonResult result = new JsonResult();
	   	...
	   	try{
	   		...
	   		HashMap getFileMap = FileUploadUtil.uploadExcelFiles(request, "dev");
	   		setMap.put("tblNm", (String)request.getParameter("tblNm"));
	   		
	   		List dataList = new ArrayList();
	    	List colsList = new ArrayList();
	    	
	    	List<HashMap> colList = devDbService.selectBefTblColList(setMap); //컬럼 목록
