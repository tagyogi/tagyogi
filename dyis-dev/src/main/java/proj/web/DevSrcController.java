...
public class DevSrcController {
	...
	public ModelAndView devSrcCreatePrcs(@RequestBody(required=false) HashMap<String, Object> paramMap) throws Exception {
			...
			for(int i = 0; i < fileFroms.length; i++) {
				...
				if(!fileTo.exists()) {
					...
					while ((readLine = br.readLine()) != null) {
						...
						if(readLine.indexOf("proj.service.repository") > -1) readLine = readLine.replaceAll("proj.service.repository", "proj.biz." + jobNm +".repository");
						if(readLine.indexOf("proj.service") > -1) readLine = readLine.replaceAll("proj.service", "proj.biz." + jobNm +".service");
	...
	public void devSqlCreate(String jobNm, String jobNmUpper, String fileNm, String fileNmUpper, String sqlFileNm, String tblNm, String jobContet) throws Exception {
		...
		for(int x = 0; x < tblNms.length; x++) {
			...
			List<Object> getCols = devDbService.selectBefTblColList(paramMap);
			...
			bw.write("<mapper namespace=\"proj.biz."+jobNm+".repository."+fileNmUpper+"Mapper\"> \r\n");
			bw.write("\r\n");
			...
			for(int i = 0; i < getCols.size(); i++) {
				HashMap<String, Object>getCol = (HashMap<String, Object>) getCols.get(i);
